<?php
	...
	function tdows_custom_style()
	{
		
		wp_enqueue_style('tdows-custom-style', get_template_directory_uri() . '/assets/css/custom.css');
		...
		$custom_css = '
			...
			.tdows_blog_wrap article h2, a, .single_post_article .entry-title, .nav-links a,.nav-links h6, #respond .comment-reply-title, .comments-title{
		...
		wp_add_inline_style('tdows-custom-style', $custom_css);
	}
	
	add_action('wp_enqueue_scripts', 'tdows_custom_style');
